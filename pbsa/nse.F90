...
!    common/extra_logical/master
   ...
   integer nse_debug,solverorder,numbubble
   logical ifupdate,ifsetpressure,ifzero,ifgravity,ifstochastic
    _REAL_ solvernorm,nsernu,ggravity,patm,surface_tension,ratio_t,tfactor,vfactor,zshift,ctfactor(1:3),zonexyz(1:3,1:3)
...
!      ! common /aret/aret
   ...
   !   patm=0.0d0 
      patm=660.5d0
      surface_tension=50.d0
      vfactor=1.0d0
      t_standard=1.0d0/tfactor
      ggravity=6.5d0
      kout = 1
      kout1 = 1
      nlp=10
    ...
    !  stop       
      !   if (master) print *,'ki',ki,'nzone',nzone
       ...
       do ii = 1, nzone
            ...
            if(master) then
            write(6,*),'volume,step',ii,aret(ii)*vfactor
            endif
        ...
        call nsetimer_start(10)
         ...
         !per-step printing
         if(master) then
        write(6,*)'dt, time, eng,iter ', dt*tfactor,t*tfactor,eng,kll
         endif
         ki = ki + 1
...
!      end do
 ...
 !    end do
     ...
     !     endif
           ...
           write(fname,'(a)') "time"
           open(unit=9,file=fname)
           write(9,*)'dt, time, eng,iter ', dt*tfactor,t*tfactor,eng,kll
           close(9)
         endif !(restart)
        end if  ! (master)
...
!+ [Enter a one-line description of ubroutine bcg here]
  subroutine bcg(m,n,l,np,n2,n3,istart,indexx,indexy,indexz &
      ...
      integer nbdcnd,ndimf,ne,npp,nx,ny,nz,lbdcnd,solvopt,kdm
           ...
           !         endif
              ...
              !      !         print *, 'pyp-1',i,j,a1,wt,pyp
               ...
               if(phi(i,j,k) > 0.0) then
                  ...
                  if(ifgravity .eqv. .true.) then
                  wi(i,j,k)=wi(i,j,k) - ggravity
                  endif 
    ...
    !  lbdcnd = 1
      ...
      epsz=1.d0
        ...
        do k = 1, l-1; do j = 1, n-1
            ...
            xso(:) = 0.d0
            call init_array(solvopt,epsx,epsy,epsz,-ui(1:m-1,1:n-1,1:l-1)*h*h,iv,xso)
        ...
        do k = 1, l-1; do j = 1, n-1
            ...
            xso(:) = 0.d0
            call init_array(solvopt,epsx,epsy,epsz,-vi(1:m-1,1:n-1,1:l-1)*h*h,iv,xso)
        ...
        do k = 1, l-1; do j = 1, n-1
            ...
            xso(:) = 0.d0
            call init_array(solvopt,epsx,epsy,epsz,-wi(1:m-1,1:n-1,1:l-1)*h*h,iv,xso)
...
!+ [Enter a onesubroutine matvec here]
  ...
  !    ! common /aret/aret
     ...
     _REAL_ ft(n2),fn(n2),fm(n2),fvec(n3),ubc2(n2),bf(n3),fnt(n3)
      ...
      pi = 4.*datan(1.0d0)
      ct = 4.d0*pi/3.d0
      if(ifgravity .eqv. .true. .and. numbubble .eq. 1) then
      ct=ct*(2.0d0*surface_tension+patm+ggravity*(zshift+zb))
      else
      ct=ct*2.0d0*surface_tension
      endif
      ct0=ct
      if(solverorder .eq. 1) then
      nlp=10
      ...
      do k=1,l-1
         do j=1,n-1
           ...
           call nsetimer_start(6)
                  ...
                  fvec(3*n22-2) = pt-2.0d0*surface_tension*curv-2.0d0*bci1
                   
                  if(ifgravity .eqv. .true. .and. numbubble .eq. 1) then
                  ubc2(n22)=pin+2.0d0*surface_tension*curv+2.0d0*bci1-patm
                  else
                  ubc2(n22)=pin+2.0d0*surface_tension*curv+2.0d0*bci1
                  endif
          !        write(444444,*) ubc2(n22),n22
                  ...
                  if(ifgravity .eqv. .true. .and. numbubble .eq. 1) then
                  fvec(3*n22-2) = fvec(3*n22-2) +patm-pin
                  else 
            ...
            if(ifsetpressure .eqv. .true.) then
               pi=3.1415926535
                ct=4.0d0*pi/3.0d0
                  ...
                  if(ifgravity .eqv. .true. .and. numbubble .eq. 1) then
                  ct=ct*(2.0d0*surface_tension+patm+ggravity*(zb+zshift))
                  ifzero=.false.
                  else
                  ct=ct*2.0d0*surface_tension
                  endif
               ...
               if(ifgravity .eqv. .true. .and. numbubble .eq. 1) then
               ubc2(i)=ct/aret(kdm)+2.0*surface_tension*cinfo((i-1)*np+16)-patm
               else
               ubc2(i)=ct/aret(kdm)+2.0*surface_tension*cinfo((i-1)*np+16)
               endif
               enddo
...
!      ! common /radius/r0
   ...
   !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ [Enter a one-line description of subroutine phiset2 here]
   subroutine phisetgravity(m,n,l,x,y,z,phi,h)
      implicit none
      _REAL_ x(0:m),y(0:n),z(0:l),phi(0:m,0:n,0:l)
      _REAL_ r0,a0,b0,c0,xt,yt,zt,xt1,xt2,yt1,yt2,zt1,zt2,phi1,phi2,h
      integer m,n,l,i,j,k
!      ! common /radius/r0
      ...
      do k=0,l
         do j=0,n
            do i=0,m
               ...
               !           call rose(7,xt,yt,zt,r0,re,phi3)
               call rose(6,xt,yt,zt,0.0d0,phi1)
               !           call rose(6,xt,yt1,r0,0.0d0,phi1)
               !           call rose(5,xt,yt2,re,0.0d0,phi2)
               !           call rose(6,xt,yt1,r0,0.09d0,phi4)
               !            call rose(5,xt,yt2,r0,0.04d0,phi5)
...
END FUNCTION
  ...
  !    ! common /aret/aret
   ...
   !   write(9,*) e
      ...
      integer ilr,info,nx,ny,nz,solvopt
...
!
      ...
      !       write(*,*) 'bbb',ui(1,1,1)
        ...
        if(master) then
         ...
         do j = 1, n-1; do i = 1, m-1
               ...
               p(i,j,0)=pe(rnu,t,x(i),y(j),z(0)) 
               ui(i,j,l-1) = ui(i,j,l-1 )- pe(rnu,t,x(i),y(j),z(l))
               p(i,j,l)=pe(rnu,t,x(i),y(j),z(l))
         end do; end do
         ...
         do k = 1, l-1; do i = 1, m-1
               ...
               p(i,0,k)=pe(rnu,t,x(i),y(0),z(k))
               ui(i,n-1,k) = ui(i,n-1,k ) - pe(rnu,t,x(i),y(n),z(k))
               p(i,n,k)=pe(rnu,t,x(i),y(n),z(k))
         end do; end do
         ...
         do k = 1, l-1; do j = 1, n-1
               ...
               p(0,j,k)=pe(rnu,t,x(0),y(j),z(k))
               ui(m-1,j,k) = ui(m-1,j,k ) - pe(rnu,t,x(m),y(j),z(k))
               p(m,j,k)=pe(rnu,t,x(m),y(j),z(k))
         end do; end do
        endif

         if (ifgravity .eqv. .true.) then
         do j = 1, n-1; do i = 1, m-1
               ui(i,j,1 ) = ui(i,j,1 )-ggravity*(z(l)-z(0)) 
               p(i,j,0)=ggravity*(z(l)-z(0)) 
               ui(i,j,l-1) = ui(i,j,l-1 )
               p(i,j,l)=0
         end do; end do
         ...
         do k = 1, l-1; do i = 1, m-1
               ui(i,1,k) = ui(i,1,k) - ggravity*(z(l)-z(k))
               p(i,0,k)=ggravity*(z(l)-z(k))
               ui(i,n-1,k) = ui(i,n-1,k ) - ggravity*(z(l)-z(k))
               p(i,n,k)=ggravity*(z(l)-z(k))
         end do; end do
      
         ! i=0 and i=xm+1 faces
      
         do k = 1, l-1; do j = 1, n-1
               ui(1,j,k) = ui(1,j,k ) - ggravity*(z(l)-z(k))
               p(0,j,k)=ggravity*(z(l)-z(k))
               ui(m-1,j,k) = ui(m-1,j,k ) - ggravity*(z(l)-z(k))
               p(m,j,k)=ggravity*(z(l)-z(k)) 
         end do; end do
        endif
      ...
      epsz=1.d0
            ...
            call allocate_array(solvopt) 
            xso(:) = 0.d0
            call init_array(solvopt,epsx,epsy,epsz,-ui(1:m-1,1:n-1,1:l-1),iv,xso)
