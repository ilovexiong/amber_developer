   ...
   do m = 1, mpdec
      ...
      do level = 1, nfocus
         ...
         if ( ipb >= 1 .and. ipb <=3 ) then
            ...
            call pb_iimdrv( npbstep,npbgrid,nstlim,1,natom,npbopt,solvopt,level,nfocus,bcopt,&
               ...
               bv,cphi,xs(xsoffset) )
#else
            write(6,'(a)') "PB_IIM is not supported in SANDER";call mexit(6,1)
#endif
         else if ( ipb == 5 ) then
            if ( ligand .or. multiblock ) then
...
#else
            write(6,'(a)') "PB_AUG is not supported in SANDER";call mexit(6,1)
#endif
         else
      ...
      if ( eneopt == 1 ) then
         ...
         if ( frcopt == 1 ) then ! only force option supported for eneopt==1
            ...
            write(6,'(a)') 'PB Bomb in pb_force(): unsupported frcopt for eneopt=1'
            call mexit(6, 1)
         end if
      ...
      ! delete fd reaction energy and forces for all close pairs
         ...
         if ( frcopt == 2 ) then
            ...
            write(6,'(a)') 'PB Bomb in pb_force(): unsupported frcopt for eneopt=3'
            call mexit(6, 1)
         end if
      ...
      ! option 4: compute fd energy and force by the new P3M option (aiming for CUDA)
         ...
         if ( pbverbose .and. pbprint ) then ! dumping forces here
            ...
            do i=1,3
               write(102,*) sum(pbfrc(i,1:natom))/dble(natom)
            enddo
            close (102)
         end if
