...
!write(4602,'(a,4f10.5)') '  H', xi(1:3), sqrt(xi(1)**2+xi(2)**2+xi(3)**2)
      ...
      if ( xi(1) > x(i) ) then
         df(1) = (phi(i+1,j,k)-phi(i,j,k))*rh*epsx(i,j,k)
      else if ( xi(1) < x(i) ) then
         df(1) = (phi(i,j,k)-phi(i-1,j,k))*rh*epsx(i-1,j,k)
      else
         df(1) = ZERO
      end if
      if ( xi(2) > y(j) ) then
         df(2) = (phi(i,j+1,k)-phi(i,j,k))*rh*epsy(i,j,k)
      else if ( xi(2) < y(j) ) then
         df(2) = (phi(i,j,k)-phi(i,j-1,k))*rh*epsy(i,j-1,k)
      else
         df(2) = ZERO
      end if
      if ( xi(3) > z(k) ) then
         df(3) = (phi(i,j,k+1)-phi(i,j,k))*rh*epsz(i,j,k)
      else if ( xi(3) < z(k) ) then
         df(3) = (phi(i,j,k)-phi(i,j,k-1))*rh*epsz(i,j,k-1)
      else
         df(3) = ZERO
      end if
